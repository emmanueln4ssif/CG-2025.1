...
const player = {
    ...
    speed: 50, // Velocidade aumentada 10x (de 5 para 50)
    isOnGround: false,
    height: 2,
    radius: 0.5,
    jumpForce: 10,
...
// Atualização do jogador com movimento corrigido e colisão melhorada
function updatePlayer(delta) {
    ...
    // Input do teclado (sinais invertidos)
    if (keyboard.pressed("W")) direction.z += 1;  // Invertido de -= para +=
    if (keyboard.pressed("S")) direction.z -= 1;  // Invertido de += para -=
    if (keyboard.pressed("A")) direction.x += 1;  // Invertido de -= para +=
    if (keyboard.pressed("D")) direction.x -= 1;  // Invertido de += para -=
